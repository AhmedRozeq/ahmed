...
const ExplanationModal: React.FC<ExplanationModalProps> = ({ isOpen, onClose, item, type, cefrLevel, register }) => {
  ...
  const titleText = type === 'grammar' ? 'Analisi Grammaticale' : 'Spiegazione';
  const Icon = type === 'grammar' ? BookOpenIcon : LightbulbIcon;
  const loadingMessage = type === 'grammar' ? 'Analisi grammaticale in corso...' : 'Caricamento spiegazione...';
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-slate-200/80 dark:border-slate-700/80">
          <h2 className="text-lg font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
            ...
            {titleText} di: <span className="font-bold text-sky-600 dark:text-sky-400">"{item}"</span>
          </h2>
