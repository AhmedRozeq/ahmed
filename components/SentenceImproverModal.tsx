      ...
      <>
        {parts.map((part, index) =>
          part.toLowerCase() === collocation.toLowerCase() ? (
            <span key={index} className="bg-sky-100 dark:bg-sky-500/20 text-sky-700 dark:text-sky-300 font-semibold rounded px-1 py-0.5">
              {part}
            </span>
          ) : (
            part
    ...
    >
      ...
      >
        <header className="flex items-center justify-between p-4 border-b border-slate-200/80 dark:border-slate-700/80 flex-shrink-0">
          <h2 id="improver-title" className="text-lg font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
        ...
        <div className="p-6 overflow-y-auto space-y-4">
          <div>
            <label htmlFor="sentence-input" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
            ...
            <textarea
              ...
              className="w-full h-24 p-2 border border-slate-300/80 dark:border-slate-600/80 rounded-md bg-white/60 dark:bg-slate-900/40 focus:ring-2 focus:ring-sky-500"
              rows={3}
            />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label htmlFor="collocation-input" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                ...
                <input
                  ...
                  className="w-full p-2 border border-slate-300/80 dark:border-slate-600/80 rounded-md bg-white/60 dark:bg-slate-900/40 focus:ring-2 focus:ring-sky-500"
                />
              </div>
              <div>
                <label htmlFor="cefr-level-select" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                  Livello Linguistico (QCER)
                </label>
                <select id="cefr-level-select" value={localCefrLevel} onChange={(e) => setLocalCefrLevel(e.target.value)} className="w-full p-2 border border-slate-300/80 dark:border-slate-600/80 rounded-md bg-white/60 dark:bg-slate-900/40 focus:ring-2 focus:ring-sky-500">
               ...
               <div>
                <label htmlFor="register-select" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
                  Registro
                </label>
                <select id="register-select" value={register} onChange={(e) => setRegister(e.target.value)} className="w-full p-2 border border-slate-300/80 dark:border-slate-600/80 rounded-md bg-white/60 dark:bg-slate-900/40 focus:ring-2 focus:ring-sky-500">
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-6 py-2 text-base font-semibold text-white bg-sky-600 rounded-lg hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sky-500 disabled:bg-slate-400 disabled:cursor-not-allowed flex items-center gap-2"
            >
          ...
          <div className="mt-4 pt-4 border-t border-slate-200/80 dark:border-slate-700/80">
            ...
            {result && (
              <div className="space-y-4 animate-fade-in">
                <div>
                  <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Frase Migliorata:</h3>
                  <p className="mt-2 text-lg text-slate-800 dark:text-slate-100 p-3 bg-slate-50 dark:bg-slate-900/40 rounded-md">
                    {highlightCollocation(result.improved_sentence, result.collocation_used)}
                  </p>
                </div>
                <div>
                  <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Spiegazione:</h3>
